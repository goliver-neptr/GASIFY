...
const gasify = (address, callback) => {
  ...
  request({ url: geocode_url, json: true }, (error, response) => {
    if (error) {
      ...
      const gplaces_url = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?rankby=distance&keyword=gas&location=' + geocode_data.latitude + ',' + geocode_data.longitude + '&type=gas_station&key=' + config.api_authentication.google_apikey
